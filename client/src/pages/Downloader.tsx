...
export default function Downloader() {
  ...
  async function download() {
    ...
    try {
      const r = await fetch("/api/dl/link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: url.trim(), email: subEmail || undefined }),
      });
      const d = await r.json();
      if (r.status === 402) {
        setShowSubModal(true); setDlError("");
        fetch("/api/dl/quota").then(r => r.json()).then(setQuota).catch(() => {});
        return;
      }
      if (!r.ok) { setDlError(d.error || "Download failed"); return; }
  ...
  function paystackPay() {
    ...
    const handler = (window as any).PaystackPop?.setup({
      ...
      callback: async (response: any) => {
        ...
        // Grant sub server-side (auto-grant on successful payment ref)
  ...
  const canDownload = !!info && (isSubscribed || quota.remaining > 0);
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden mb-4">
            ...
            <div className="px-4 pb-4">
              ...
              <button onClick={download}
                ...
                {dlLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Generating link…</> : <><Download className="w-4 h-4" /> Download MP4</>}
              </button>
              {!isSubscribed && quota.remaining === 0 && (
                <p className="text-center text-yellow-400 text-xs mt-2">
                  Free limit reached —{" "}
                  <button onClick={() => setShowSubModal(true)} className="underline hover:text-yellow-300">Subscribe for Ksh 100/month</button>
                </p>
              )}
            </div>
          </div>
